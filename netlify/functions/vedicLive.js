...
exports.handler = async (event) => {
  try {
    ...
    // Placeholder: Fetch NASA planetary data (for Sidereal conversion)
    ...
    // PDF Generation
    const pdfPath = path.join('/tmp', 'Vedic_Report.pdf');
    const pdf = new PDFDocument();
    pdf.pipe(fs.createWriteStream(pdfPath));

    pdf.fontSize(20).fillColor('#7dd3fc').text('Sathyadarshana Vedic Astrology Report', 50, 50);
    pdf.fontSize(14).fillColor('#e0f2fe').text(`Name: ${name}`, 50, 100);
    pdf.text(`DOB: ${dob} ${tob}`, 50, 120);
    pdf.text(`Place: ${place}`, 50, 140);
    pdf.moveDown();

    pdf.fontSize(12).fillColor('#38bdf8').text('Planetary Data (Sidereal Approx):');
    planets.forEach((pl, i) => {
      pdf.text(`${i+1}. ${pl.name} - Gravity: ${pl.gravity}`);
    });

    pdf.fontSize(28).fillColor('#32cd32').opacity(0.1)
    ...
    return {
      ...
      body: JSON.stringify({
        planets,
        pdfUrl: 'https://your-netlify-site.netlify.app/Vedic_Report.pdf'
      })
    };
  } catch (err) {
